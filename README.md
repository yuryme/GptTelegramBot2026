...
- Create reminders prefer a strict normalized `schedule` object with ISO datetimes; legacy expression fields remain as fallback/context.
...
Доступ к VPS для служебных команд выполняется по SSH-ключу без хранения пароля в репозитории:

```bash
ssh root@5.255.125.171 "hostname"
```

Управление сервисом с Windows выполняется через `scripts/bot_service.bat`; скрипт использует `ssh -o BatchMode=yes` и требует заранее настроенный ключ в `~/.ssh/authorized_keys` на сервере.
